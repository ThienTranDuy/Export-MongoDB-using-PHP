...
/**
 * MongoDB Active Record Library
 *
 * A library to interface with the NoSQL database MongoDB. For more information see http://www.mongodb.org
 */

/**
 * @property Mongo $connection  Mongo Connection object.
 * @property MongoDB $db     MongoDB database object
 */
class MyMongoDriver {
    ...
    private $persist_key = 'st_mongo_persist';
    ...
    public $groups = array(); // Public to make debugging easier
    private $limit = 999999;
    private $offset = 0;
    private $mongo_return = "array"; // array of object
    // If you are having problems connecting try changing this to TRUE
    private $dbhostflag = TRUE;
     ...
     */
    public function __construct($mongodbname, $mongohost = 'localhost', $mongoport = 27017) {

        if (!class_exists('MongoClient')) {
            show_error("The MongoDB PECL extension has not been installed or enabled", 500);
        }
        if (is_array($mongodbname)) {
            $this->host = $mongodbname['mongo_host'];
            $this->dbname = $mongodbname['mongo_db'];
            $this->port = $mongodbname['mongo_port'];
            $this->user = $mongodbname['mongo_user'];
            $this->pass = $mongodbname['mongo_pass'];
        } else {
            $this->host = $mongohost;
     ...
     * 	@usage: $instance->drop_collection('foo', 'bar');
     */
     ...
     *
     * 	@usage: $instance->select(array('foo', 'bar'))->get('foobar');
     */
     ...
     * 	criteria.
     *
     * 	@usage : $instance->group(array('foo' => 'bar'))->get('foobar');
     */
    public function group($groups, $value = null) {
        if (is_array($groups)) {
     ...
     * 	criteria.
     *
     * 	@usage : $instance->where(array('foo' => 'bar'))->get('foobar');
     */
    public function where($wheres, $value = null) {
        if (is_array($wheres)) {
            foreach ($wheres as $wh => $val) {
                if (is_array($val) && isset($this->wheres[$wh])) {
                    $this->wheres[$wh] = array_merge($this->wheres[$wh], $val);
                } else {
                    $this->wheres[$wh] = $val;
                }
            }
     ...
     *
     * 	@usage : $instance->or_where(array('foo'=>'bar', 'bar'=>'foo'))->get('foobar');
     */
    public function or_where($wheres = array()) {
        ...
        //thắng viết
    public function or_where_all($wheres = array()) {
        if (count($wheres) > 0) {
            if (!isset($this->wheres['$or']) || !is_array($this->wheres['$or'])) {
                $this->wheres['$or'] = array();
            }
...
////
     ...
     *
     * 	@usage : $instance->where_in('foo', array('bar', 'zoo', 'blah'))->get('foobar');
     */
     ...
     *
     * 	@usage : $instance->where_gt('foo', 20);
     */
     ...
     *
     * 	@usage : $instance->where_gte('foo', 20);
     */
     ...
     *
     * 	@usage : $instance->where_lt('foo', 20);
     */
     ...
     *
     * 	@usage : $instance->where_lte('foo', 20);
     */
     ...
     *
     * 	@usage : $instance->where_between('foo', 20, 30);
     */
     ...
     *
     * 	@usage : $instance->where_between_ne('foo', 20, 30);
     */
     ...
     *
     * 	@usage : $instance->where_ne('foo', 1)->get('foobar');
     */
     ...
     * 	a line.
     *
     * 	@usage : $instance->like('foo', 'bar', 'im', FALSE, TRUE);
     */
     ...
     * 	set to 1 (ASC).
     *
     * 	@usage : $instance->order_by(array('foo' => 'ASC'))->get('foobar');
     */
     ...
     *
     * 	@usage : $instance->get_where('foo', array('bar' => 'something'));
     */
    public function get_where($collection = "", $where = array(), $returns = array()) {
        return ($this->where($where)->get($collection, $returns));
    }
     ...
     * 	Get the documents based upon the passed parameters
     *
     * 	@usage : $instance->get('foo');
     */
    public function get($collection = "", $returns = array()) {
        if (empty($collection)) {
    ...
    public function distinct($collection = "", $key = "") {
        ...
        $documents = $this->db->{$collection}->distinct($key, $this->wheres);
     ...
     *
     * 	@usage : $instance->aggregate('foo');
     */
    public function aggregate($collection = "", $returns = array()) {
        if (empty($collection)) {
            exit("In order to retreive documents from MongoDB, a collection name must be passed");
        }
        $pipeline = array();
        ...
        $documents = $this->db->{$collection}->aggregate($pipeline);

        // Clear
        $this->_clear();
        $finalreturns = array_merge($returns, $documents["result"]);

        if ($this->mongo_return == 'object') {
            return (object) $finalreturns;
        } else {
            return $finalreturns;
        }
    }

    public function aggregate_pipline($collection = "", $pipeline = array(), $returns = array()) {
        if (empty($collection)) {
            exit("In order to retreive documents from MongoDB, a collection name must be passed");
        }
    ...
    public function aggregate_pipline_usedisk($collection = "", $pipeline = array(), $returns = array()) {
        if (empty($collection)) {
            exit("In order to retreive documents from MongoDB, a collection name must be passed");
        }
        $options = array("allowDiskUse" => true);
        $documents = $this->db->{$collection}->aggregate($pipeline, $options);
     ...
     *
     * 	@usage : $instance->insert('foo', $data = array());
     */
     ...
     *
     * 	@usage: $instance->update('foo', $data = array());
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->inc(array('num_comments' => 1))->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->dec(array('num_comments' => 1))->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->set('posted', 1)->update('blog_posts');
     * 	@usage: $instance->where(array('blog_id'=>123))->set(array('posted' => 1, 'time' => time()))->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->unset('posted')->update('blog_posts');
     * 	@usage: $instance->where(array('blog_id'=>123))->set(array('posted','time'))->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->addtoset('tags', 'php')->update('blog_posts');
     * 	@usage: $instance->where(array('blog_id'=>123))->addtoset('tags', array('php', 'codeigniter', 'mongodb'))->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->push('comments', array('text'=>'Hello world'))->update('blog_posts');
     * 	@usage: $instance->where(array('blog_id'=>123))->push(array('comments' => array('text'=>'Hello world')), 'viewed_by' => array('Alex')->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->pop('comments')->update('blog_posts');
     * 	@usage: $instance->where(array('blog_id'=>123))->pop(array('comments', 'viewed_by'))->update('blog_posts');
     */
     ...
     *
     * 	@usage: $instance->pull('comments', array('comment_id'=>123))->update('blog_posts');
     */
     ...
     * 	Renames a field
     *
     * 	@usage: $instance->where(array('blog_id'=>123))->rename_field('posted_by', 'author')->update('blog_posts');
     */
     ...
     *
     * 	@usage : $instance->delete_all('foo', $data = array());
     */
     ...
     *
     * 	@usage : $instance->command(array('geoNear'=>'buildings', 'near'=>array(53.228482, -0.547847), 'num' => 10, 'nearSphere'=>true));
     */
     ...
     * 	set to 1 (ASC).
     *
     * 	@usage : $instance->add_index($collection, array('first_name' => 'ASC', 'last_name' => -1), array('unique' => TRUE));
     */
     ...
     * 	set to 1 (ASC).
     *
     * 	@usage : $instance->remove_index($collection, array('first_name' => 'ASC', 'last_name' => -1));
     */
     ...
     *
     * 	@usage : $instance->remove_all_index($collection);
     */
     ...
     *
     * 	@usage : $instance->list_indexes($collection);
     */
     ...
     *
     * 	@usage : $instance->get_dbref($object);
     */
    public function get_dbref($obj) {
        if (empty($obj) OR ! isset($obj)) {
            $this->show_error('To use MongoDBRef::get() ala get_dbref() you must pass a valid reference object', 500);
        }
     ...
     *
     * 	@usage : $ref = $instance->create_dbref($collection, $id);
     */
    ...
    private function connect() {
        $options = array('socketTimeoutMS'=>"360000");

        if ($this->persist === TRUE) {
            $options['persist'] = isset($this->persist_key) && !empty($this->persist_key) ? $this->persist_key : 'st_mongo_persist';
        }
